  ...
  <div>
    <header class="navbar">
      ...
      <nav class="nav-links">
        ...
        <button @click="logout" class="nav-link logout-button" v-if="isAuthenticated">
          Cerrar sesión
        </button>

        <!-- Link login si NO está autenticado -->
        <a href="/login" class="nav-link login-link" v-else>Iniciar sesión</a>
      </nav>
    </header>
